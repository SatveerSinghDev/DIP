...
for(i in 1:nrow(pad.filter))
{
  for(j in 1:ncol(pad.filter))
  {
    ...
    pad.filter[i,j]=1/(1+(5/dist)^(2*2))#gaussian high pass
    #pad.filter[i,j]=1/(1+(dist/5)^(2*2))#gaussian low pass
  }
}
...
save.image(as.cimg(final.fft.mat),"/home/satveer/Documents/sem6/DIP/IMAGES/pfiltered.png")
...
save.image(as.cimg(new.final.fft.mat),"/home/satveer/Documents/sem6/DIP/IMAGES/cfiltered.png")
